	...
	<div class="grid_6 alpha omega divide welcome">
		...
		<div class="main">
			...
			{if !$secretOK}
				...
				{elseif $matchC < #unlock2# && $match_your && $get.degree!=0}
					{assign var='y' value=#unlock2#}
					{block name=statusbar}{/block}
				{elseif $matchC < #unlock3#}
					{assign var='y' value=#unlock3#}
					{block name=statusbar}{/block}
				{elseif $matchC < #unlock4# && $match_your && $get.degree!=0}
			...
			{if $get.degree!=0}
				{if $statusOK || $get.status=="a"}
					...
					{/if}
					<div class="subtext">
						{assign var="link" value="http%3A%2F%2Flikebright.com%2Fcupid%2F%3Fuid%3D{$fbook.uid}%26degree={$get.degree}%26status={$get.status}"}
						{if not $match_your}You currently have no matches.<br/>{/if}
						<a target="_blank" href="http://www.facebook.com/dialog/feed?app_id=189260257772056&to={$guid}&redirect_uri=http%3A%2F%2Flikebright.com%2Fcupid%2Fclose.html&link={$link}&caption= &name=Play likebright with me!">
							 Invite more friends
						</a> to add their networks and votes!
					</div>
				{else}
					<div class="subtext">
						<a target="_blank" href="http://www.facebook.com/dialog/feed?app_id=189260257772056&to={$guid}&redirect_uri=http%3A%2F%2Flikebright.com%2Fcupid%2Fclose.html&link={$link}&caption= &name=Play likebright with me!">
			...
			{else}
				<div class="subtext">
					Want your top matches?<br/>
					<a href="?degree=1{$url.status}">Vote on friends' top matches!</a>
				</div>
			{/if}
	...
	<div class="grid_6 alpha omega divide">
		{if $matchC >= #unlock1# || $secretOK}
			<div class="grid_3 alpha tops">
				<div class="title">
					{assign var="cTitle" value="{if $get.degree=="0"}The most matched guys in my network{else}My most matched guy friends{/if}{if $get.status=="x"} with single or unknown relationship status{/if}."}
			...
			<div class="grid_3 omega tops">
				<div class="title">
					{assign var="cTitle" value="{if $get.degree=="0"}The most matched girls in my network{else}My most matched girl friends{/if}{if $get.status=="x"} with single or unknown relationship status{/if}."}
